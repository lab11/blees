...
void lps331ap_readPressure (float* pres) {
    ...
    *pres = (0x00FFFFFF & (((uint32_t)pressure_data[2] << 16) |
            ((uint32_t) pressure_data[1] << 8) |
            ((uint32_t) pressure_data[0]))) / 4096.0;
}

// Note: does not return correct temperature
void lps331ap_readTemp (float* temp) {
    ...
    // I dont even...
    *temp = (((int16_t) (0x0000FFFF & (((uint32_t) temp_data[1] << 8) | ((uint32_t) temp_data[0])))) / 480.0) + 42.5;
}
...
void lps331ap_config_interrupt (interrupt_config interrupt_c_1,
                                interrupt_config interrupt_c_2,
                                bool active_low) {
    ...
    // bradjc: Not sure why we do this...
    uint8_t data[1] = {0x00};
...
static void read_reg (uint8_t* data, uint8_t reg) {
...
void lps331ap_read_controlreg1 (uint8_t* data) {
    read_reg(data, CTRL_REG1);
}

void lps331ap_read_controlreg2 (uint8_t* data) {
    read_reg(data, CTRL_REG2);
}

void lps331ap_read_controlreg3 (uint8_t* data) {
    read_reg(data, CTRL_REG3);
}

void lps331ap_read_status_reg (uint8_t* data) {
    read_reg(data, STATUS_REG);
}

void lps331ap_read_interrupt_source_reg(uint8_t* data) {
    read_reg(data, INT_SOURCE_REG);
}
