...
static void test_temp (int16_t temp) {
    si7021_read_humidity_and_temp(temp_finish_measurement);
}

bool begin_advertising = false;
static void temp_finish_measurement(uint16_t humidity, int16_t temperature) {
    ...
    */
    ...
    //begin_advertising = true;
    adv_config_data();
    nrf_gpio_pin_clear(6);
}

static void temp_take_measurement() {

    // STARTING TEMP TAKE MEASUREMENT
    //nrf_gpio_pin_clear(6);
...
*/
    ...
    //si7021_reset();
    //si7021_heater_off();
    si7021_init(&twi_instance, temp_take_measurement);
...
*/
    ...
    err_code = app_timer_start(m_temp_timer_id, APP_TIMER_TICKS(10, APP_TIMER_PRESCALER), NULL);
