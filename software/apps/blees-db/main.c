...
// DB Variables
#define DB_WORKING_MEM_SIZE 2000
uint8_t db_memory[DB_WORKING_MEM_SIZE];
...
static void lux_take_measurement(void * p_context)
{
    ...
    nrf_drv_twi_enable(&twi_instance);
    tsl2561_on();
    app_timer_start(m_lux_wait_timer_id, (uint32_t)(APP_TIMER_TICKS(800, APP_TIMER_PRESCALER)), NULL);
}
...
static void db_init () {
    db_query_mm_t mm;
    init_query_mm(&mm, db_memory, DB_WORKING_MEM_SIZE);
    parse("CREATE TABLE sensors (id int, pressure_Pa int, humidity int, temp_C int, light_lx int);", &mm);
}
